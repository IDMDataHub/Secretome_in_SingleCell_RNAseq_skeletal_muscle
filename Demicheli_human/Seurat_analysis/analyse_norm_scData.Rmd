...
setwd(paste0(work_dir,"Analyse_NormData_with_Seurat/"))
...
####
#### If you let GEOquery download it for you:
####

raw_file <- getGEOSuppFiles(GEO = "GSE143704",
...
metadata <- read_delim( rownames(metadatafile))
metadata <- metadata[,c(1:6)]
...
#### Extract only Fibroblasts subtypes samples

```{r extract_one_cellType}
...
samples_Fibro1 %>% length
samples_Fibro2 %>% length
samples_Fibro3 %>% length


##> Counts only of these samples :
srat_counts_fibro <- srat_data@assays$RNA@counts[,c(samples_Fibro1,samples_Fibro2,samples_Fibro3)]

## For which genes ?
srat_counts_fibro %>% rownames %>% length # 21701

#saveRDS(srat_counts_fibro,"srat_counts_fibroblasts_3grp.rds")

fibro_counts_var_genes <- srat_counts_fibro[most_var_genes,]
