...
work_dir <-"/projects/Secretome_in_Single_Cell_skeletal_muscle/DeMicheli_human/"
...
#                                  verbose = TRUE)
# saveRDS(muscle.markers,"muscle_markers.rds")

##>replaced by :
muscle.markers <- readRDS("Seurat_analysis/muscle_markers.rds")

# muscle.markers %>% pull(cluster) %>% as.factor %>% levels
# muscle.markers %>% group_by(cluster) %>% slice_max(n=2,order_by = avg_log2FC,with_ties=FALSE )
...
DoHeatmap(srat_data.norm, features = top10Up$gene,size=3) + NoLegend()
...
#### Venn between fibroblasts gene-markers :
